...
class EditorViewModel(
    ...
    fun openUri(uri: Uri) {
        viewModelScope.launch {
            discardAssistantBitmap()
            _uiState.update {
                it.copy(
                    sourceUri = uri,
                    isAssistantCapture = false,
                    isLoadingImage = true,
                    isRunningOcr = false,
                    ocrResult = null,
                    errorMessage = null,
    ...
    /** Accepts an assistant-provided image. It is memory-only and never has a URI. */
    fun openBitmap(bitmap: Bitmap) {
        discardAssistantBitmap()
        ocrRepository.clear()
        _uiState.update {
            it.copy(
                sourceUri = null,
                isAssistantCapture = true,
                bitmap = bitmap,
                isLoadingImage = false,
                isRunningOcr = false,
                ocrResult = null,
    ...
    private fun discardAssistantBitmap() {
        val previous = _uiState.value
        if (previous.isAssistantCapture) {
    ...
    fun setOcrProfile(profile: OcrProfile) {
        if (_uiState.value.ocrProfile == profile) return
        // A result from another pipeline must never be reused after switching.
        ocrRepository.clear()
        ocrProfilePreferences.save(profile)
        _uiState.update { it.copy(ocrProfile = profile, ocrResult = null) }
        runOcr(force = true)
    }
    ...
    private fun runOcr(force: Boolean) {
        ...
        val profile = _uiState.value.ocrProfile
        viewModelScope.launch {
            _uiState.update { it.copy(isRunningOcr = true, errorMessage = null) }
            runCatching { ocrRepository.recognize(current, language, profile, force) }
                .onSuccess { result ->
                    _uiState.update {
