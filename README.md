...
The default `高速（標準）` model is the bundled ML Kit Japanese/Latin recognizer;
it is entirely offline and is designed for quick assistant use. The OCR text
bottom sheet can switch to `高精度（遅い）`, PP-OCRv6_medium, for difficult small
text and manga. The selection is saved locally and the accurate model remains
available without a download. Both pipelines support the dedicated vertical
